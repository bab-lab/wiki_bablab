# Onboarding

----------

## First Steps

If you are a new member of the BAB Lab, there are a few basic things you will want to set up before or on your first day. 

**Here are some tips:**

 1. Read the [Lab Manual](https://paper.dropbox.com/doc/BAB-Lab-Manual-AhbI86OnRmh6Lu5Osk6g6bBAg-Y1PCLFS9hIQt2CsZrDDJy#:h2=Server)

 2. Ask the lab manager to be added to the following
- [Slack](https://slack.com/) (download the desktop and mobile [apps][3])
- [Trello][4] (download the desktop and mobile [apps][5] - watch this [tutorial][6])
- Google calendars
- Email list
- Dropbox paper
- Relevant OSF projects

3. Send (via Slack) the lab manager your information including your
- Preferred name
- Preferred email
- Phone number
- Photo
- Brief bio for the lab website

 4. Complete the onboarding process for your position below


<hr>

### Onboarding - Staff Research Associate

1. Submit to Bridget
    - Signed employment contract
2. Contact HR
    - Human Resources Coordinator
    - 1283A Franz Hall
    - (310) 206-9720
3. Submit to HR
    - [Union overtime/comp form][7]
    - [Personal data form][8]
    - [Background check authorization form][9]
4. Schedule with HR
    - Background check phone call
    - Hiring meeting
    - Bring employment verification [documents][10] to meeting (i.e. passport)
    - Sign [state oath of allegiance/patent policy/patent acknowledgment][11] (in person)
5. Pick-up
    - Location: Psychology Main Office (1285 Psychology Building – See Tyler Tuione)
        - Parking permits
6. Respond
    - To the tracker I-9 email on or before your first day of work
7. Create (once you have your employee ID)
    - Create a [UCLA logon ID][12]
    - Create a [UCPath account][13] (payroll, benefits, etc.)
    - Create an At Your Service Online ([AYSO][14]) account (retirement)
8. Visit (once you have your employee ID)
    - Location: UCLA BruinCard Center (Kerckhoff Hall, Room 123)
        - Bring ID and completed [form][15]
9. Complete (once you have your UCLA logon ID)
    - Sign-up and complete the required employee training [courses][16]
    - Sign-up and attend [orientation][17]
    - Upload orientation training certificates to Dropbox (BAB/Lab/Training)
10. Select
    - Health insurance plan (within 30 days)
Retirement plan (within 90 days)
Union membership requires pension plan
11. Pritzker Access
    - Email Tyler Tuione – tuione@psych.ucla.edu
    - Include your name and Bruincard # to be granted weekend swipe card access as well as B and C level access for freezer storage
    - The swipe access reader is located on the right hand side of door to the right courtyard of the tower entrance.
12. IRB Trainings
    - Create a [UCLA SSO for CITI Program][18]
    - Add and complete the following courses:
        - Human Research – Social & Behavioral Researchers & Staff
        - Human Research- Biomedical Researchers & Staff
        - UCLA HIPAA
    - Add certificates to the training folder on Dropbox (BAB/Lab/Training)
    - Get a WebIRB account
        - Email your faculty sponsor/advisor the following information:
            - Your UCLA Logon ID – (Verify your [UCLA Logon ID][19])
            - Your UCLA UID # (9-digit)
            - Your full name
                - First
                - Middle
                - Last
            - Your email address
            - Your department and division
        - Bridget to email this information to webIRBHelp@research.ucla.edu to request the account.
        - Ask the lab manager to be added to all IRB protocols
13. IBC Trainings
    - Sign up for the following courses via UCLA [WorkSafe][20]
        - NIH Guidelines for UCLA Researchers IBC Compliance Training (online)
        - Laboratory Safety Fundamentals (online)
        - Blood-borne Pathogens Training (online)
        - Medical Waste Management (online)
        - Biosafety ABC’s – Biosafety Level 2 Training (in-person)
        - Biosafety Cabinet (online)
    - Add certificates to your user folder on Dropbox (BAB/Lab/Training)
    - Record completion for [HPL][21]
    - Submit certificates to Arielle Radin (radina02@g.ucla.edu) at HPL
    - Read the [Lab Specific Biosafety Manual][22] and sign off
    - Complete Lab Specific Training and sign off
        - This must be updated annually
    - Get vaccinations (suggested)
        - Visit OHF at 67-120 CHS x56771
        - Recommended vaccines
            - Hepatitis B
            - Flu (Influenza)
            - MMR (Measles, Mumps & Rubella)
            - Varicella (Chickenpox)
            - Tdap (Tetanus, Diptheria, Pertussis)
            - Meningococcal
14. REDCap
    - Complete and send REDCap access form to Martin Lai (mylai@mednet.ucla.edu) (Dropbox/BAB/Lab/Lab_protocols/REDCap/Access/Template/)
15. Website admin access 
    - Contact Jun Wan (jwan@psych.ucla.edu) for access to the [life sciences Wordpress multisite server][23]
16. MRI Trainings
    - TBD 
17. Department of Psychology Printing Acess
    - Login (upper right hand corner) to: https://support.lifesci.ucla.edu/hc/en-us
    - click submit a request
    - inquire about gaining printer access- include your UID number and email
    - access printing at Franz Hall 

<h4>Important links:</h4>

- [UCLA time reporting system][24]

<h4>Review:</h4>

- [Getting started at UCLA][25]
- [Welcome Kit][26]
- [How-to access UCPath portal][27]
- [Workers’ Comp
Pamphlet][28]
- [When an injury occurs][29]
- [Substance Abuse Brochure][30]

<hr>

### Onboarding - Volunteer Research Assistant

<hr>

### Onboarding - Postdoctoral Scholar

<hr>

### Onboarding - Graduate Student


  [1]: 
  [2]: 
  [3]: https://slack.com/downloads/mac
  [4]: https://trello.com/emilyanntowner/boards
  [5]: https://trello.com/en-US/platforms
  [6]: https://www.youtube.com/watch?v=_Ry-SnJygy8&feature=youtu.be
  [7]: https://ucla.app.box.com/s/z58tkq6l13qwl0zw5gtqhdxq0cne1wcu
  [8]: https://ucla.app.box.com/s/7jmouwl8fbp5039qq4tfmreo5r5ydmjq
  [9]: https://ucla.app.box.com/s/tothrcm0zcz50hj829bkcj1idclcudzr
  [10]: https://ucla.app.box.com/s/iwguajwkedo2zf2lfr5ie3z5tc4vnz7m
  [11]: https://ucnet.universityofcalifornia.edu/forms/pdf/upay-585.pdf
  [12]: https://accounts.iam.ucla.edu/#/
  [13]: https://idpproxy-ucpath.universityofcalifornia.edu/simplesaml/module.php/ucpathdiscovery/disco.php?entityID=https://ucpath.universityofcalifornia.edu&return=https://idpproxy-ucpath.universityofcalifornia.edu/simplesaml/module.php/saml/sp/discoresp.php?AuthID=_6a3d8a7c8144ccec21e8fff0206d805c7b4d0beb08%253Ahttps%253A%252F%252Fidpproxy-ucpath.universityofcalifornia.edu%252Fsimplesaml%252Fsaml2%252Fidp%252FSSOService.php%253Fspentityid%253Dhttps%25253A%25252F%25252Fucpath.universityofcalifornia.edu%25253A443%25252Fsimplesaml%25252Fmodule.php%25252Fsaml%25252Fsp%25252Fmetadata.php%25252Fdefault-sp%2526cookieTime%253D1563390873%2526RelayState%253Dhttps%25253A%25252F%25252Fucpath.universityofcalifornia.edu%25252Fsaml_login&returnIDParam=idpentityid
  [14]: https://atyourserviceonline.ucop.edu/ayso/
  [15]: https://secure.bruincard.ucla.edu/BCW/BruinCard_Web/Docs/BC%20Terms%20%20Signature%2006.pdf
  [16]: https://ucla.app.box.com/s/mizhokn39tq7z6odvnvutvaoko11n823
  [17]: https://www.chr.ucla.edu/training-and-development/new-employee-orientation
  [18]: http://ora.research.ucla.edu/OHRPP/Documents/Education/SSO_CITI_New_Acct.pdf
  [19]: https://accounts.iam.ucla.edu/lookup
  [20]: https://worksafe.ucla.edu/Ability/Programs/Standard/Control/elmLearner.wml?PortalID=LearnerWeb
  [21]: https://docs.google.com/document/d/1hCYg4hYJ7wi4nsLl1vDs1a-7Of3Tnzvocx58bdcj2cc/edit
  [22]: https://www.dropbox.com/s/t9atbx9d991gbwa/Lab_specific_biosafety_manual.pdf?dl=0
  [23]: https://sites.lifesci.ucla.edu/
  [24]: https://uctrs.it.ucla.edu/
  [25]: https://www.chr.ucla.edu/new-employee/getting-started
  [26]: https://ucnet.universityofcalifornia.edu/forms/pdf/welcome-kit.pdf
  [27]: https://www.centralresourceunit.ucla.edu/s/
  [28]: https://ucla.app.box.com/s/jyzoag8v9qw6katuvgegjil8an2tsx2j
  [29]: https://ucla.app.box.com/s/nua4ypfpjlt1226fusney4zyvo6qzzhj
  [30]: https://ucla.app.box.com/s/qrj4j7bnca1r8fy9n1bdfdf6orf1g0dq